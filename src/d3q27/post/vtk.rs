use crate::d3q27::D;
use crate::d3q27::{Lattice, ShallowLattice};
        ...
        lines.zip(self.nodes.iter_mut()).for_each(|(line, node)| {
            ...
            if parts.len() == 2 * D {
                let coordinates: [Float; D] = [
                    ...
                    parts[4].parse().expect("Invalid coordinates value"),
                    parts[5].parse().expect("Invalid coordinates value"),
                ];
        ...
        lines.zip(self.nodes.iter_mut()).for_each(|(line, node)| {
            ...
            if parts.len() == D {
                let velocity: [Float; D] = [
                    ...
                    parts[2].parse().expect("Invalid velocity value"),
                ];
...
impl ShallowLattice {
    pub fn from_data(time_step: usize, case_parameters: &CaseParameters) -> ShallowLattice {
        let mut lattice = ShallowLattice::new(
            case_parameters.nx,
            case_parameters.ny,
            case_parameters.nz,
            1.0,
...
where
    P: AsRef<Path>,
{
    let point_data = lattice.nx * lattice.ny * lattice.nz;
    ...
    lattice.nodes.iter().for_each(|node| {
        ...
        let velocity_z = node.velocity[2];
        writeln!(
            file,
            "{velocity_x:>.6e} {velocity_y:>.6e} {velocity_z:>.6e}"
        )
        .unwrap();
    });
    ...
    lattice.nodes.iter().for_each(|node| {
        ...
        let physical_velocity_z = node.velocity[2] * case_parameters.velocity_conversion_factor;
        writeln!(
            file,
            "{physical_velocity_x:>.6e} {physical_velocity_y:>.6e} {physical_velocity_z:>.6e}"
        )
        .unwrap();
    });
