...
impl Simulation {
    pub fn build_case_setup() -> io::Result<Simulation> {
        ...
        let case_setuo_path_str = case_setup_path.to_str().unwrap();
        let simulation: Simulation;
        if case_setup_path.exists() {
            println!(
                "Reading the case setup file: {}.\n",
                case_setuo_path_str.yellow().bold()
            );
...
impl Simulation {
    pub fn print_residuals(&self, residuals: &Residuals) {
        ...
        println!(
            "{:>8} {:>16.8e} {:>16.8e} {:>16.8e}",
            self.time_step, residuals.density, residuals.velocity[0], residuals.velocity[1]
        );
    }
...
set terminal pop"#,
    ...
    pub fn write_data_from_steps(&self, lattice: &ShallowLattice) {
        ...
        if let Err(e) = fs::create_dir_all(&step_path) {
            eprintln!(
                "Error while creating the step {} directory: {e}.",
                &self.time_step
            );
            process::exit(1);
        };
...
impl Lattice {
    fn write_coordinates(&self) -> io::Result<()> {
        ...
        let mut file = File::create(path)?;
        writeln!(file, "{:>8} {:>8} {:>16} {:>16}", "i", "j", "x", "y")?;
        for j in 0..self.ny {
            for i in 0..self.nx {
                writeln!(
                    file,
                    "{i:>8} {j:>8} {x:>16.8e} {y:>16.8e}",
                    x = self.get_node(&[i, j]).coordinates[0],
                    y = self.get_node(&[i, j]).coordinates[1],
                )?;
            }
        }
        Ok(())
...
west_boundary_condition          = dirichlet 1.0 0.1 0.0
...
pub fn read_bounce_back_map() -> io::Result<Vec<Vec<i32>>> {
    ...
    for j in 0..lattice.ny {
        for i in 0..lattice.nx {
            writeln!(
                file,
                "{density:>16.8e}",
                density = lattice.get_node(&[i, j]).density,
            )?;
        }
    }
    Ok(())
...
{
    let mut file = File::create(path)?;
    writeln!(file, "{:>16} {:>16}", "velocity_x", "velocity_y")?;
    for j in 0..lattice.ny {
        for i in 0..lattice.nx {
            writeln!(
                file,
                "{velocity_x:>16.8e} {velocity_y:>16.8e}",
                velocity_x = lattice.get_node(&[i, j]).velocity[0],
                velocity_y = lattice.get_node(&[i, j]).velocity[1],
            )?;
        }
    }
    Ok(())
    ...
    P: AsRef<Path>,
{
    ...
    writeln!(file, "POINTS {} float", lattice.nx * lattice.ny)?;
    for j in 0..lattice.ny {
        for i in 0..lattice.nx {
            writeln!(
                file,
                "{:>.4e} {:>.4e} 0.0",
                lattice.get_node(&[i, j]).coordinates[0],
                lattice.get_node(&[i, j]).coordinates[1]
            )?;
        }
    }
    writeln!(file, "POINT_DATA {}", lattice.nx * lattice.ny)?;
    writeln!(file, "SCALARS density float 1")?;
    writeln!(file, "LOOKUP_TABLE default")?;
    for j in 0..lattice.ny {
        for i in 0..lattice.nx {
            writeln!(file, "{:>.8e}", lattice.get_node(&[i, j]).density)?;
        }
    }
    ...
    for j in 0..lattice.ny {
        for i in 0..lattice.nx {
            writeln!(
                file,
                "{:>.8e} {:>.8e} 0",
                lattice.get_node(&[i, j]).velocity[0],
                lattice.get_node(&[i, j]).velocity[1]
            )?;
        }
    }
    Ok(())
...
fn y_axis_reverse_matrix(matrix: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
